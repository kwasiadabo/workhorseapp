...
import { useServiceCategories, useDeleteServiceCategory } from './useServiceCategories';
...
export default function ServiceCategoriesTab() {
  ...
  const { data, isLoading, isError } = useServiceCategories({ page, limit: 20, sort: 'displayOrder' });
  ...
  return (
    <div>
      ...
      {canManage && (
        <>
          <ServiceCategoryFormDialog open={formOpen} onOpenChange={setFormOpen} category={editingCategory} />
          <ConfirmDialog
            open={Boolean(deleteTarget)}
            onOpenChange={(open) => !open && setDeleteTarget(null)}
            title="Delete this category?"
            description={`This will permanently remove "${deleteTarget?.name}". Services in this category will become uncategorized.`}
            confirmLabel="Delete"
